...
public class SemanticAnalysisVisitor implements ObjectVisitor {
	...
	// Exp e1,e2;
		public Object visit(Divide n) {
			...
			ASTNode scope = st.getScope();
			MethodDecl m;
			
			if(scope instanceof MethodDecl) {
				m = (MethodDecl) scope;
				div = m.i.s;
				if(n.e2 instanceof IdentifierExp) {
					IdentifierExp id = (IdentifierExp)n.e2;
					denominator = id.s;
				}
			}
			int val = -2;
			...
			if(val == 0) {
				report_error(n.getLineNo(), "Division by zero not allowed");
				return null;
			}
		...
		if ( c != null && c != "" ) {
			...
			if ( ms != null ) {
				...
				//for(int i = 0;i<ms.getParameters().size();i++) {
					//idToValue.put(ms.getParameters().get(i).getName(), ms.getParameters().get(i).)
				//}
				if ( ms.getParameters().size() != n.el.size() ) {
					...
					for (int i = 0; i < n.el.size(); i++) {
						if(n.el.get(i) !=null && n.el.get(i) instanceof IntegerLiteral) {
							IntegerLiteral integ = (IntegerLiteral) n.el.get(i);
							String str = ms.getName();
							if(str.equals(div)) {
								if(ms.getParameters().get(i).getName().equals(denominator) && integ.i ==0)
								{
									zerodiv = ms.getParameters().get(i).getName();
									report_error(n.getLineNo(),"Division by zero not allowed");
								}
							}
						}
